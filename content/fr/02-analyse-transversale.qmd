...
if 'df_cross' in locals() and not df_cross.empty:
    ...
    # ---------------------------------------------------------
    sns.scatterplot(ax=axes[0], data=df_cross, x='gdp', y='co2_per_capita', 
    ...
    # ---------------------------------------------------------
    sns.scatterplot(ax=axes[1], data=df_cross, x='gdp', y='co2_per_capita', 
                    color='#c0392b', alpha=0.5, s=50, edgecolor='white')
    
    # Tendance LOWESS pour guider l'œil
    sns.regplot(ax=axes[1], data=df_cross, x='gdp', y='co2_per_capita', 
...
if DATA_LOADED and not df_cross.empty:
    ...
    # ═══════════════════════════════════════════════════════════════
    sns.scatterplot(ax=axes[0,1], data=df_cross, x='gdp', y='resid_abs',
                    alpha=0.6, s=60, color='#e74c3c')
    
    # Tendance LOWESS
    from statsmodels.nonparametric.smoothers_lowess import lowess
    lowess_result = lowess(df_cross['resid_abs'], np.log(df_cross['gdp']), frac=0.3)
    ...
    # ═══════════════════════════════════════════════════════════════
    top_erreurs = df_cross.nlargest(10, 'resid_abs')[['country', 'erreur_tonnes', 'gdp']]
...
if DATA_LOADED and not df_cross.empty:
    ...
    surpred = df_cross[df_cross['resid_cub'] > 0.5]  # Surestimés (prédit > observé)
    souspred = df_cross[df_cross['resid_cub'] < -0.5]  # Sous-estimés
    
    display(Markdown(f"""
### Profils des pays mal prédits

**Pays surestimés** (modèle prédit trop de CO₂) :
- **Nombre** : {len(surpred)}
- **Exemples** : {', '.join(surpred.nlargest(5, 'resid_cub')['country'].tolist())}
- **PIB moyen** : {surpred['gdp'].mean():,.0f} $/hab
- **Interprétation** : Pays ayant décarbonné **plus vite** que la moyenne EKC
  (ex: nucléaire français, hydroélectricité nordique)

**Pays sous-estimés** (modèle prédit trop peu de CO₂) :
- **Nombre** : {len(souspred)}
- **Exemples** : {', '.join(souspred.nsmallest(5, 'resid_cub')['country'].tolist())}
- **PIB moyen** : {souspred['gdp'].mean():,.0f} $/hab
- **Interprétation** : Pays dépendants des **énergies fossiles** malgré leur richesse
  (ex: économies pétrolières, charbon)

**Test de normalité** :  
- Jarque-Bera = {jb_stat:.2f} (p = {jb_pval:.3f})
- Conclusion : {"Résidus normalement distribués ✓" if jb_pval > 0.05 else "Résidus NON gaussiens ⚠️ (queue épaisse)"}
"""))
```
...
### Test F emboîté : le terme cubique est-il nécessaire ?
...
**Critère AIC** : Cubique ({AIC_CUB:.1f}) vs Quadratique ({AIC_QUAD:.1f})

- Gain : **{AIC_QUAD - AIC_CUB:.1f}** points
"""))
```

## Résultat clé n°1 : U inversé renforcé
...
- Le terme cubique est **positif et significatif** → Courbe en **N**.
- Le découplage est **temporaire** : rebond aux très hauts revenus.
""")
    else:
        print(f"""
**Coefficient $\\beta_3 = {BETA3:.4f}** (p = {PVAL3:.4f})

- Le terme cubique est **non significatif** → le modèle quadratique suffit.
""")
```
...
if DATA_LOADED and not df_cross.empty:
    gdp_grid = np.linspace(df_cross['gdp'].min(), df_cross['gdp'].max(), 200)
    ...
    ax.scatter(df_cross['gdp'], df_cross['co2_per_capita'], 
...
1. **Significativité statistique** : Le test F (p=0.020) et le coefficient β₃** 
   (p=0.011) indiquent que le terme cubique améliore l'ajustement aux données 
   de 2020.

2. **Capacité prédictive** : La validation croisée révèle que ce gain 
   statistique ne se traduit pas par une meilleure généralisation (RMSE +1.4%).

**Interprétation** : Le terme cubique capture probablement des spécificités 
des pays ultra-riches (Qatar, Koweït, Singapour) qui constituent des cas 
particuliers (économies rentières, cités-États) plutôt qu'une tendance 
structurelle généralisable.
...
Cependant, nous reconnaissons que les données de 2020 ne permettent pas 
d'exclure définitivement un déclin accéléré (β₃ < 0) aux très hauts revenus. 
Des travaux futurs avec des panels temporels multi-pays permettraient de 
...
"""))
            ...
            print("Impossible de calculer les points d'inflexion.")
...
"""))
        ...
        print(f"Le terme cubique n'est pas significatif (p = {PVAL3:.3f}) — le modèle quadratique suffit.")
```
