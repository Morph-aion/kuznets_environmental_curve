...
# MAPPING VARIABLES (Fix pour éviter NameError si _setup.qmd est en français)
...
# Calcul de sécurité pour GDP_MEAN_USA si pas défini
if 'GDP_MEAN_USA' not in locals() and 'df_usa' in locals() and not df_usa.empty:
    ...
    GDP_MEAN_USA = 0 # Fallback pour éviter crash
```
...
if DATA_LOADED and not df_usa.empty:
    ...
    def interprete_adf(pval_levels, pval_diff, threshold=0.05):
        ...
        if pval_levels < threshold:
            return "I(0)", "✅ Stationary in levels"
        elif pval_diff < threshold:
            return "I(1)", "✅ Non-stationary in levels, stationary in difference"
        else:
            return "Ambiguous", "⚠️ Inconclusive tests (possibly I(2) or structural break)"
...
*Reminder: H₀ = "presence of a unit root (non-stationary)". If p < 0.05, we reject H₀ → stationary series (I(0)).*
"""))
    ...
    if concl_co2 == concl_gdp == "I(1)":
        # IDEAL CASE
        display(Markdown("""
**✅ Interpretation**: Both series are **I(1)** (non-stationary in levels, stationary in first difference).
...
"""))
        ...
        # BOTH I(0)
        display(Markdown("""
**✅ Interpretation**: Both series are **I(0)** (stationary in levels).
...
"""))
        ...
        display(Markdown(f"""
**⚠️ DIAGNOSTIC: Integration asymmetry detected**
...
"""))
        ...
        # AMBIGUOUS CASE
        display(Markdown(f"""
**⚠️ DIAGNOSTIC: Inconclusive ADF tests**
...
3. **I(2) series** (unlikely for GDP and CO₂)
...
**🔍 Robustness test (ln(GDP) with alternative specification)**
...
**Interpretation**: {
    "Both specifications converge → robust result." 
    if (adf_gdp[1] < 0.05) == (adf_gdp_no_trend[1] < 0.05)
    else "**Divergence detected** → ln(GDP) is likely **trend-stationary** (I(0) around a deterministic trend, not I(1)). The ARDL model remains valid."
}
"""))
```
...
1. **AR(1)**: Pure autoregressive model (without $\text{GDP}$) — minimal benchmark
...
```{python}
#| label: tbl-ardl
#| tbl-cap: "ARDL Models — United States (1990-2020)"
#| echo: false
...
- The cubic model has the best R² ({m_ardl_cub.rsquared:.3f}), 
  but $\\beta_3$ is not significant (p = {p_val_cub:.3f}).
- AIC favors the **{best_aic_model}** model.

**→ We must arbitrate between statistical significance and parsimony. 
...
→ Too few data points to robustly estimate a cubic ARDL model (4 parameters).
...
results = [
    ...
    metrics('ARDL — AR(1)', y_test, y_pred_ar),
    metrics('ARDL — Linear', y_test, y_pred_lin),
    metrics('ARDL — Quadratic', y_test, y_pred_quad),
    metrics('ARDL — Cubic', y_test, y_pred_cub),
]
...
rmse_lin = get_rmse('ARDL — Linear')
rmse_quad = get_rmse('ARDL — Quadratic')
rmse_cub = get_rmse('ARDL — Cubic')
...
| **Key coefficient** | — | $\\beta_2$: p={pval_b2:.3f} | $\\beta_3$: p={pval_b3:.3f} |
| **Status** | — | {txt_sig_b2} | {txt_sig_b3} |
...
| **Durbin-Watson** (autocorrelation) | {dw_stat:.2f} | — | {dw_verdict} |
...
Extreme persistence of emissions → Shocks are quasi-permanent. With only 30 observations, 
the estimator is biased upwards (Nickell bias). The true value is likely $\\rho \\approx 0.90–0.95$.
"""))
...
→ +1\\% GDP **{verb_SR}** emissions by **{effect_SR_pct:.2f}\\%** (year 1).
...
**Verdict**: {
    ...
    if elasticity_LR < 0 
    else f"**Persistent coupling** — the decoupling observed in cross-section is not yet structural over time."
}
"""))
```
...
if rho >= 0.99:
    display(Markdown("""
**Turning point: Not calculable** ($\\rho \\approx 1$ → no defined long-term equilibrium)
...
→ {"**Passed**" if gdp_usa_current > gdp_turning else "**Not reached**"} (${distance_pct:+.1f}$\\%)

{
    ...
    else "Contradiction with data (drop since 2005) → the model smoothes post-2010 breaks."
}
"""))
    else:
        display(Markdown(f"""
**Incoherent turning point** (${int(gdp_turning):,}$ \$/capita, out of realistic range 10k–100k).  
**Empirical observation**: Peak ~2005-2007 (~45k–50k \$/capita).
"""))

else:
    display(Markdown(f"""
**No turning point** ($\\beta_2 = {beta_pib2:.4f} \\geq 0$, no inverted U).  
Contradiction with observed data → post-2010 decoupling is too recent for this global model.
"""))
```
...
ax.set_ylabel("CO₂ per capita (tons)", fontsize=12)
...
| Year | Predicted CO₂ | 95% CI | Cumulative Variation |
|:-----:|:----------:|:------:|:-----------------:|
""" + "\n".join([
...
1. **Increasing uncertainty**: The confidence interval widens (±{(df_future.iloc[-1]['co2_upper'] - df_future.iloc[-1]['co2_lower'])/2:.1f} t in 2025)
2. **Structural stability hypothesis**: Assumes the GDP-CO₂ relationship estimated over 1990-2020 remains valid
...
**→ These projections are **conditional extrapolations**, not certain forecasts.**
"""))
```
...
1. **Retained model**: Quadratic ARDL  
   → Cubic term not significant ($p = {m_ardl_cub.pvalues.get('I(log_gdp_c ** 3)', 1):.3f}$) 
   → N-curve **not detected** over 1990-2020
...
**→ The US trajectory does NOT show a rebound (N-curve), but {final_conclusion} 
